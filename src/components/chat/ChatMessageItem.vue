		...
		<div class="chat-msg-normal" v-show="msgInfo.type!=10" :class="{'chat-msg-mine':mine}">
			...
			<div class="chat-msg-content">
				<div class="chat-msg-top">
					<!--
          <span v-if="ipAddress!=null && ipAddress!=undefined && ipAddress!=''">({{ipAddress}})
						-->
          </span><span>{{showName}}</span>
				...
				<div class="chat-msg-bottom" @contextmenu.prevent="showRightMenu($event)">
					...
					<div class="chat-msg-image" v-if="msgInfo.type==1">
						<div class="img-load-box" v-loading="loading" element-loading-text="전송중.." element-loading-background="rgba(0, 0, 0, 0.4)">
							<img class="send-image" :src="JSON.parse(msgInfo.content).thumbUrl" @click="showFullImageBox()" />
						</div>
						<span title="전송실패" v-show="loadFail" @click="handleSendFail" class="send-fail el-icon-warning"></span>
					</div>
...
<script>
	...
	export default {
      ...
      ipAddress: {
		...
		methods: {
			handleSendFail() {
				this.$message.error("파일 전송에 실패했습니")
			},
			showFullImageBox() {
