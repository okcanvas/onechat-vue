<template>
  <div class="box">
    <el-popover
      ...
      <p v-if="heartCount!=null">설정시간: <span v-text="heartCount"></span></p>
      <p v-if="connectorDate!=null">연결일시:<br/> <span v-text="connectorDate"></span></p>
      <div v-if="state===1">
        <p style="color: green;">연결됨</p>
      </div>
      <div v-if="state===0">
        <hr/>
        <p v-if="closeTimeDate!=null">종료시간:<br/> <span v-text="closeTimeDate"></span></p>
        <p v-if="reConnectCount!=null">재시도: <span v-text="reConnectCount"></span></p>
        <p style="color: red;" v-if="errorMsg!=null">종료됨<br/> <span v-text="errorMsg"></span></p>
      </div>
...
<script>
    ...
    computed: {
      title() {
        ...
        return "시그널"+String(this.node.signal);
      },
